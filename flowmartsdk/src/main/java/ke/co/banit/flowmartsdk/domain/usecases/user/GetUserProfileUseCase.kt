...
/**
 * Use case for retrieving the user profile in the FlowMart API.
 *
 * This use case encapsulates the logic for invoking the [UserRepository] to fetch the user's profile data,
 * providing a consistent and reusable approach for retrieving profile information.
 *
 * @property repository An instance of [UserRepository] used to fetch user profile data.
 */
internal class GetUserProfileUseCase(private val repository: UserRepository) {
     ...
     */
    suspend operator fun invoke(): Result<UserResponse, Exception> {
